...
def add_arguments():
    ...
    parser.add_argument("--limitTrain", dest = "limitTrain", type = float, default = 1.0)
    return parser
...
class ResNet(pl.LightningModule):
    ...
    def configure_optimizers(self):
      ...
      sched = CosineAnnealingLR(opt, T_max=T_max,eta_min=learnRate, last_epoch=args.maxEpochs)
...
def main(args):
    ...
    if args.gpu == True:
        trainer = pl.Trainer(limit_train_batches=args.limitTrain, accelerator = "gpu", profiler  = "simple", devices = args.devices, max_epochs = args.maxEpochs, log_every_n_steps = 1, callbacks = [dvcStats,lr_monitor,checkpoint_callback,EarlyStopCB_Time , TQDMProgressBar(refresh_rate=3)], logger = tb_logger)
    else:
        trainer = pl.Trainer(limit_train_batches=args.limitTrain, max_epochs = args.maxEpochs, profiler = "simple", log_every_n_steps = 1,callbacks = [dvcStats,lr_monitor,checkpoint_callback,EarlyStopCB_Time ], logger = tb_logger) #, TQDMProgressBar(refresh_rate=3)]
    ...
    epochnum = (checkpoint_callback.best_model_path)[6:7]
    print(ysl)
    print(psl)
    ...
    with open(logpath+'lightning_logs/'+'/params.txt', 'a') as f:
        ...
        f.write(confmat(psl,ysl))
        f.write("################################################################################\n")
    with open(logpath+'lightning_logs/'+'/standardized.txt', 'a') as f:
        f.write(str(args.modeltype) + '- seed: ' + str(args.seed) + "AUC,ACC,EPOCH:" +str(roc_auc_score(ysl, psl)) + "||" + str(torchmetrics.Accuracy(threshold=0.5).to(torch.device("cpu"))(psl, ysl)) +"||" + epochnum)
    disp = RocCurveDisplay.from_predictions(ysl,psl)
    plt.savefig(logpath+"lightning_logs/" + curtim + " - " + args.modeltype + " - " + str(args.seed))
    return 0
