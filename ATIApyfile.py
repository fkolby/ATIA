...
def add_arguments():
    ...
    parser.add_argument("--effectivebatchSize", dest = "batchSize", type = int, default = 2)
...
class ResNet(pl.LightningModule):
    def __init__(
        ...
        self.relu = nn.ReLU(inplace=True)
        if args.modeltype != "Patch": self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        x = self.bn1(x)
        x = self.relu(x)
        if args.modeltype != "Patch": x = self.maxpool(x)
    ...
    def mixup_fn(self,x,y):
      ...
      if x.size(dim=0)%2 != 0:
        ...
        yngt = torch.reshape(torch.tensor(y[0]),shape=(1,1))
    ...
    def train_dataloader(self):
      return torch.utils.data.DataLoader(image_datasets['train'], batch_size = global_batch_size,
                                             shuffle=True, num_workers=workers, pin_memory=True)
    def val_dataloader(self): 
      return torch.utils.data.DataLoader(image_datasets['val'], batch_size = global_batch_size,
...
#channels = 3?
def ResNet50(num_classes, epochs, block, batch_size, l1kernelSize = 7,l1stride = 2, l1pad = 3):
    return ResNet(Bottleneck, block,epochs, num_classes, batch_size, l1kernelSize = l1kernelSize, l1stride=l1stride, l1pad=l1pad)
...
#earlyStop based on compute?
#setting seed?
#log auc, roc, accuracy
#implement setting values in layer
#seed datamodule dataset to replicate
testsetname = args.testSetName

EarlyStopCB_Time = EarlyStopping('timer_', patience = 0, mode="max")

print("got here")
num_training_steps_per_epoch = len(image_datasets['train'])/max_epochs
...
tb.configure(argv = [None, f'--logdir={logdir}',f"--port={args.port}"])
url = tb.launch()
...
if args.gpu == True:
    trainer = pl.Trainer(accelerator = "gpu", profiler  = "simple", devices = args.devices, max_epochs = max_epochs, log_every_n_steps = 1, callbacks = [dvcStats,lr_monitor,checkpoint_callback,EarlyStopCB_Time , TQDMProgressBar(refresh_rate=3)], logger = tb_logger)
else:
    trainer = pl.Trainer(max_epochs = max_epochs, strategy = "ddp", profiler = "simple", log_every_n_steps = 1,callbacks = [dvcStats,lr_monitor,checkpoint_callback,EarlyStopCB_Time ], logger = tb_logger) #, TQDMProgressBar(refresh_rate=3)]
if args.modeltype == "baseCase":
    
    timestart = time.time()
    flopCalcedModelBase = ResNet50(num_classes=2, block=[3,3,9,3],epochs = max_epochs, batch_size = global_batch_size, l1pad = 3, l1stride = 2, l1kernelSize = 7)
    ...
    flopCalcedModelLowKern = ResNet50(num_classes=2, block=[3,3,9,3],epochs = max_epochs, batch_size = global_batch_size, l1pad = 2, l1stride = 2, l1kernelSize = 4)
    ...
    flopCalcedModelPatch = ResNet50(num_classes=2, block=[3,3,9,3],epochs = max_epochs, batch_size = global_batch_size, l1pad = 0, l1stride = 4, l1kernelSize = 4)
...
predloader = torch.utils.data.DataLoader(image_datasets[testsetname], batch_size = global_batch_size,
                                            shuffle=False, num_workers=workers, pin_memory=True)
