...
def add_arguments():
    ...
    parser.add_argument("--data_dir", dest = "data_dir", type=str, default='/home/frederik/Documents/ATIA/chestXray/chest_xray')#default on comp: ISICdataNewAndLovely/jpeg/train/ xray: /home/frederik/Documents/ATIA/chestXray/chest_xray
...
def main(args):
    ...
    PneumoniapathBase = "/home/frederik/Documents/ATIA/Results/lightning_logs/PneumoniaPatch/2-Mon Oct 24 10:18:09 2022/checkpoints/epoch=35-step=2484.ckpt"

    ch = "/home/frederik/Documents/ATIA/Results/Melanoma/Patch-/lightning_logs/Patch/2-Mon Oct 17 08:07:45 2022/checkpoints/epoch=60-step=549.ckpt"
    if args.modeltype == "baseCase":
        print("hi")
        model = ResNet50(num_classes=2,args=args,ds=ds, layers=[3,3,9,3],epochs = args.maxEpochs, batch_size = args.batchSize, l1pad = 3, l1stride = 2, l1kernelSize = 7).load_from_checkpoint(ch)
        print("hi2")
    if args.modeltype == "4x4":
        print("nas")
        model = ResNet50(num_classes=2,args=args,ds=ds, layers=[3,3,9,3],epochs = args.maxEpochs, batch_size = args.batchSize, l1pad = 1, l1stride = 2, l1kernelSize = 4).load_from_checkpoint(PneumoniapathBase)
    if args.modeltype == "Patch":
        print("ho")
        model = ResNet50(num_classes=2,args=args,ds=ds, layers=[3,3,9,3],epochs = args.maxEpochs, batch_size = args.batchSize, l1pad = 0, l1stride = 4, l1kernelSize = 4).load_from_checkpoint(PneumoniapathBase)

    predloader = torch.utils.data.DataLoader(ds[args.testSetName], batch_size = args.batchSize,
                                                shuffle=False, num_workers=args.workers, pin_memory=True)
    preds = trainer.predict(model,predloader, ckpt_path=PneumoniapathBase)
    ...
    ysl = ysl.type(torch.int)
    confmat = torchmetrics.ConfusionMatrix(2)
    confmat(psl,ysl)
    print(ds[args.testSetName].class_to_idx)
    print(confmat(psl,ysl))
    """
    with open(logpath+'lightning_logs/'+'/params.txt', 'a') as f:
        ...
        f.write("Time to run model:" +str(-9999) + "\n")
